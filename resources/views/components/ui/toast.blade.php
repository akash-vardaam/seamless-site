...
<script>
  ...
  window.toast = function(options) {
    ...
    toastElement.className = `toast ${typeClass} rounded-lg px-4 py-3 shadow-lg flex flex-col gap-2 min-w-[280px]`;
    ...
    if (description || message) {
      content += `<div class="text-sm opacity-90">${description || message}</div>`;
    }
    ...
    toastElement.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        ...
        <button onclick="removeToast('${toastId}')" class="text-xs font-bold opacity-70 hover:opacity-100 transition-opacity flex-shrink-0">×</button>
      </div>
      ${action ? `<div class="flex gap-2 mt-1">${action}</div>` : ''}
    `;
